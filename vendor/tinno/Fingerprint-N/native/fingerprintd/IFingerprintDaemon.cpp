...
status_t BnFingerprintDaemon::onTransact(uint32_t code, const Parcel& data, Parcel* reply,
    ...
    //guomingyi add start.
        case SET_MSG:{
            if(getFpChipName() == SILEAD 
                ||getFpChipName() == ELAN
                ||getFpChipName() == GOODIX
                ||getFpChipName() == CHIPONE) {
                CHECK_INTERFACE(IFingerprintDaemon, data, reply); 
                const int32_t msg = data.readInt32();
                setMsg(msg);  
                reply->writeInt32(0);
            }
            return NO_ERROR;
        ...
        case GET:{
            CHECK_INTERFACE(IFingerprintDaemon, data, reply); 
            const int32_t arg = data.readInt32();
            const int32_t ret = get(arg);  
            reply->writeNoException();
            reply->writeInt32(ret);
            return NO_ERROR;
        }
