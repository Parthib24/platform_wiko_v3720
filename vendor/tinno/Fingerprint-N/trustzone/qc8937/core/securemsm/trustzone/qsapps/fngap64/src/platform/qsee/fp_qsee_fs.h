#ifndef __FP_QSEE_FS_H__
#define __FP_QSEE_FS_H__

typedef struct fp_qsee_fs_t {
    int32_t fd;
} fp_qsee_fs_t;

typedef fp_qsee_fs_t* FP_ObjectHandle;   // use it to store fd in qsee
...
extern int fp_tee_file_read(FP_ObjectHandle so_handle, uint8_t *buffer, uint32_t size, uint8_t *path);
extern int fp_tee_file_write(uint8_t *path, FP_ObjectHandle *so_handle, uint8_t *buffer, uint32_t size);
