...
static void fpsensor_spi_clk_enable(u8 bonoff)
{
    ...
    if (bonoff && (count == 0)) {
        ret = clk_prepare_enable(g_fpsensor->clk_main );
        //fpsensor_debug(DEBUG_LOG, "%s, start to clk_enable= %d.\n", __func__, ret);
        count = 1;
    } else if ((count > 0) && (bonoff == 0)) {
        clk_disable_unprepare(g_fpsensor->clk_main );
        count = 0;
    }
    //TINNO END
...
#endif
{
    ...
    fpsensor_dev->clk_main = devm_clk_get(&spi->dev, "spi-main");
    if (IS_ERR(fpsensor_dev->clk_main)) {
        dev_err(&spi->dev, "cannot get spi0 main clock or dma clock. main clk err : %ld .\n",
        PTR_ERR(fpsensor_dev->clk_main));
        return PTR_ERR(fpsensor_dev->clk_main);
    }
    #endif
    //TINNO END
