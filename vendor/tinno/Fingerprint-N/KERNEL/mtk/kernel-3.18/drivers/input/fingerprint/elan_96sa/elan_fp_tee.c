...
//TINNO END
    ...
    //Add spi clk by yinglong.tang
    if (bonoff && (count == 0)) {
        ret = clk_prepare_enable(fp->clk_main );
        count = 1;
    } else if ((count > 0) && (bonoff == 0)) {
        clk_disable_unprepare(fp->clk_main );
        count = 0;
    }
    //TINNO END
...
{	
	...
	if (fp_vendor_tee != ELAN_VENDOR) {
		ELAN_DEBUG("%s, Fingerprint vendor not elan return!\n", __func__);
		return -1;	
	}
	//add by yinglong.tang
    ...
    //Add spi clk by yinglong.tang
    #if !defined(CONFIG_MTK_CLKMGR)
        fp->clk_main = devm_clk_get(&pdev->dev, "spi-main");
        if (IS_ERR(fp->clk_main)) {
            dev_err(&pdev->dev, "cannot get spi0 main clock or dma clock. main clk err : %ld .\n",
            PTR_ERR(fp->clk_main));
            return PTR_ERR(fp->clk_main);
        }
    #endif
    //TINNO END
