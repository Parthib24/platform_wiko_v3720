...
class AInterface : public AREF_t 
{
protected:
...
public:
	...
	static int				GetRelayerInterfaceSize(void);
	static AInterface*		GetIndexedRelayerInterface(
							int		inIdx
							);
	...
	static AInterface*		HoldInf(AInterface* inInf,int inSeqNo);
	static void				UnHoldInf(AInterface* inInf,int inSeqNo);
	virtual	int				GetSvcClientSidePid();
	virtual int				IsProcessDead(pid_t in_pid);
