...
public abstract class EnrollClient extends ClientMonitor {
    ...
    public EnrollClient(Context context, long halDeviceId, IBinder token,
        ...
        if (cryptoToken == null) {
            if (DEBUG) Slog.e(TAG, "EnrollResult: fatal err!");
            return;
        }
    ...
    public boolean onEnrollResult(int fingerId, int groupId, int remaining) {
        if (groupId != getGroupId()) {
            if (DEBUG) Slog.w(TAG, "groupId != getGroupId(), groupId: " + groupId +
        ...
        if (remaining == 0) {
	     ...
	     //TINNO BEGIN, added by wenguangyu, for fingerprint
            ApeFpDataUtils.getInstance().addApeFpDataForUser(getContext(), fingerId,
                    getTargetUserId());	     
    ...
    private boolean sendEnrollResult(int fpId, int groupId, int remaining) {
        ...
        if (receiver == null) {
            if (DEBUG) Slog.e(TAG, "sendEnrollResult: receiver == null!");
        ...
        try {
            ...
            if (DEBUG) Slog.w(TAG, "Failed to notify EnrollResult:", e);
            return true;
        }
    }
    ...
    public int start() {
        ...
        if (daemon == null) {
            if (DEBUG) Slog.w(TAG, "enroll: no fingeprintd!");
            return ERROR_ESRCH;
        }
        ...
        try {
            ...
            if (result != 0) {
                if (DEBUG) Slog.w(TAG, "startEnroll failed, result=" + result);
    ...
    public int stop(boolean initiatedByClient) {
        IFingerprintDaemon daemon = getFingerprintDaemon();
        if (daemon == null) {
            if (DEBUG) Slog.w(TAG, "stopEnrollment: no fingeprintd!");
            return ERROR_ESRCH;
        }
        try {
            final int result = daemon.cancelEnrollment();
            if (result != 0) {
                if (DEBUG) Slog.w(TAG, "startEnrollCancel failed, result = " + result);
                return result;
            }
        } catch (RemoteException e) {
