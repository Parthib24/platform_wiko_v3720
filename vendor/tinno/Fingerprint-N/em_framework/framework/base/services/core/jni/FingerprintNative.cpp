...
static const char* FINGERPRINT_NATIVE = "com/android/server/fingerprint/FingerprintNative";
/*****************************************************************************************/
...
static void *monitor_thread(void *args) {
    ...
    do {
        if (fp_update_finger_state(&event) < 0) {
...
static int native_bl_ctl(JNIEnv *env, jobject object, jint set, jint brightness) {
    LOGD(TAG "[%s:%d]\n",__func__,__LINE__);
    fp_dev_ioctl(set, brightness);
    return 0;
}

static int native_get_bl(JNIEnv *env, jobject object) {
    LOGD(TAG "[%s:%d]\n",__func__,__LINE__);
    return getLcdBrightness();
}

static const JNINativeMethod g_methods[] = {
    { "native_init", "(I)I", (void *)native_init },
    { "native_bl_ctl", "(II)I", (void *)native_bl_ctl },
    { "native_get_bl", "()I", (void *)native_get_bl },
    { "native_setObj", "(Lcom/android/server/fingerprint/FingerprintNative;)I", (void *)native_setObj },
};

int register_android_server_fingerprint_FingerprintService(JNIEnv* env) {
    int result = RegisterMethodsOrDie(env, FINGERPRINT_NATIVE, g_methods, NELEM(g_methods));
    ALOG(LOG_VERBOSE, LOG_TAG, "FingerprintNative JNI ready.\n");
