...
public class FingerprintUtils {
    private static final String TAG = "FingerprintUtils-tag";
    private static final boolean DEBUG = !("user".equals(android.os.Build.TYPE));
    private static final int VIB_T = FingerprintFeatureOptions.getAuthSuccessVibTime();//SystemProperties.getInt("persist.tinno.fp.vib", 30);
    private static final long[] FP_ERROR_VIBRATE_PATTERN = new long[] {0, VIB_T, 100, VIB_T}; 
    private static final long[] FP_SUCCESS_VIBRATE_PATTERN = new long[] {0, VIB_T};
    ...
    public static void vibrateFingerprintError(Context context) {
        ...
        if (DEBUG) Log.i(TAG, "vibrateFingerprintError:"+vibrator);
        if (vibrator != null) {
    ...
    public static void vibrateFingerprintSuccess(Context context) {
        Vibrator vibrator = context.getSystemService(Vibrator.class);
        if (DEBUG) Log.i(TAG, "vibrateFingerprintSuccess:"+vibrator);
        if (vibrator != null) {
